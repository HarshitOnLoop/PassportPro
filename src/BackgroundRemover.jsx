...
const BackgroundRemover = () => {
  ...
  // --- 1. PRELOAD FROM LOCAL FOLDER ---
  useEffect(() => {
    const config = {
      // This points to the public/models folder you created
    ...
    try {
      preload(config);
      console.log("Preloading local AI models...");
    } catch (e) {
  ...
  const resizeImage = (file) => {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (event) => {
        const img = new Image();
        img.onload = () => {
          const maxWidth = 1000; // Fast speed
  ...
  const handleRemoveBg = async () => {
    ...
    try {
      ...
      // 3. Config (LOCAL MODE)
      const config = {
        publicPath: window.location.origin + "/models/", // <--- CRITICAL FIX
        model: 'small', 
        output: {
          format: 'image/png',
          quality: 0.8
      ...
      alert("Error: Could not find AI models.\n\nPlease ensure 'unet.onnx' and 'imgly_background_removal.wasm' are inside the 'public/models/' folder.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  useEffect(() => {
    if (step === 3 && printSheet) {
       // Optional: Auto-refresh logic if you save singlePhoto in state
    }
  }, [pageSize]);
  ...
  return (
    <div style={styles.pageWrapper}>
        ...
        <div style={styles.card}>
            ...
            {step === 1 && (
              <div style={styles.stepContainer}>
                 ...
                 <p style={styles.subHeading}>Upload an image to remove background.</p>
              ...
              <div style={styles.editorContainer}>
                  <div style={styles.editorLeft}>
                     <div style={styles.cropWrapper}>
                        <Cropper 
                          image={processedImage} crop={crop} zoom={zoom} rotation={rotation} aspect={3.5/4.5} 
                  ...
                  <div style={styles.editorRight}>
                     ...
                     <div style={styles.controlGroup}>
                        ...
                        <div style={styles.colorGrid}>
                           {['#ffffff', '#4a90e2', '#d0021b', '#8b572a', '#d3d3d3'].map(c => (
               ...
               <div style={styles.printContainer}>
                  ...
                  <div style={styles.downloadRow}>
                     <a href={printSheet} download={`passport-sheet-${pageSize}.jpg`} style={{textDecoration:'none'}}>
                        <button style={styles.btnPrimary}>Download Sheet</button>
                     </a>
                  </div>
      ...
      <footer style={styles.footer}>&copy; 2024 PassportPro</footer>
    </div>
  );
};
...
const styles = {
  ...
  logoIcon: { width: '32px', height: '32px', backgroundColor: '#4facfe', borderRadius: '6px' },
  ...
  colorInput: { width: '34px', height: '34px', border: 'none', background: 'none', cursor: 'pointer' },
  ...
  footer: { backgroundColor: '#1a1d21', color: '#fff', padding: '20px', textAlign: 'center', fontSize: '14px' },
